...
BaseCache* BuildCacheBank(Config& config, const string& prefix, g_string& name, uint32_t bankSize, bool isTerminal, uint32_t domain) {
    ...
    uint32_t accLat = (isTerminal)? 2 : latency; //PRIME setting
...
Buffer* BuildAcceleratorBuffer(Config &config, const string &prefix){//prefix = "sys.accelerator.buffer."
    uint32_t size = config.get<uint32_t>(prefix + "size", 1024*1024/*1MB*/);//unit: B
    uint32_t lineSize = config.get<uint32_t>(prefix + "lineSize",1024/*1KB*/);//unit: B
    uint32_t numLines = size/lineSize;
    g_string name("");
    MemObject *mems = BuildMemoryController(config,zinfo->lineSize,zinfo->freqMHz,0,name);
    Buffer *buffer = new Buffer(numLines,lineSize,mems);
    return buffer;
}


void BuildMemristorAccelerator(Config& config, const string& prefix){//prefix = "sys.accelerator."
    uint32_t tiles = config.get<uint32_t>(prefix + "tiles",16);
    string prefix_buffer = prefix + "buffer.";
    std::vector<Tile<RealDevice>*> *vTiles = new std::vector<Tile<RealDevice>*>(tiles);
    for(uint32_t i = 0; i < tiles; ++i){
        uint16_t bits = config.get<uint32_t>(prefix + "crossbar.bits",2);
        uint16_t row_size = config.get<uint32_t>(prefix + "crossbar.rowSize",128);
    ...
    zinfo->chip = new Chip<RealDevice>(vTiles);
}
...
static void InitSystem(Config& config) {
    ...
    string prefix1 = string("sys.accelerator.");
    //Init memristor
